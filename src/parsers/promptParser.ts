...
const matchMetaKeywordRegex = new RegExp(String.raw`\b(${metaKeywords.join('|')})\b`, 'g');
...
export function updatePromptState(prompt: string, caret: number): ParseResult {
    ...
    promptInfo.words.forEach((word) => {
        word.value = word.value.replaceAll('_', ' ');
    });

    // Fix weight parsing bug when delimiter contains decimal point.
    const isNumberRegex = new RegExp(/^\d+$/);
    const isEndOfWeight = new RegExp(/(.+):(\d+)$/);
    for (let i = 0; i < promptInfo.words.length - 1; i++) {
        const currWord = promptInfo.words[i];
        const nextWord = promptInfo.words[i + 1];
        if (isNumberRegex.exec(nextWord.value)) {
            const match = isEndOfWeight.exec(currWord.value);
            if (match) {
                currWord.value = match[1];
