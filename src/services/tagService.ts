...
export function initializeTagModels(resData: ResponseData | undefined): void {
    ...
    alwaysUnderscoreTags.clear();
    (opts[`${EXTENSION_ID}_always_underscore_tags`] as string).split(/[\n,]/).forEach((tag: string) => {
    ...
    alwaysSpaceTags.clear();
    (opts[`${EXTENSION_ID}_always_space_tags`] as string).split(/[\n,]/).forEach((tag: string) => {
        tag = tag.trim().replaceAll('_', ' ');
        if (tag) {
...
export function searchTag(query: string): ItemProps[] {
    const queries = query
        .toLowerCase()
        .split(/[ _-]/g)
        .filter((q) => q.trim() !== '');
    ...
    for (const key of ['0', '1', '3', '4', '5', 'custom']) {
        groupCounter[key] = opts[`${EXTENSION_ID}_max_results_group${key}`] as number;
    }
