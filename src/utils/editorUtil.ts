...
function getTagInsertionData(state: AppProps, props: ItemProps): TextInsertionData {
    ...
    const source = (opts[`${EXTENSION_ID}_tag_source`] as string).replaceAll('.', '_');
    const delimiter = (opts[`${EXTENSION_ID}_${source}_${props.category}_tag_delimiter`] as string) ?? 'auto';
    if (!db_tag.alwaysSpaceTags.has(insertTag)) {
        ...
        if (replaceToUnderscore) {
            insertTag = insertTag.replaceAll(' ', '_');
        }
    }

    if (insertionInfo.needPrependComma) {
        insertTag = ', ' + insertTag;
    } else if (startPosition > 0 && promptInfo.prompt[startPosition - 1] !== '\n' && offset <= 0 && insertionInfo.needPrependSpace) {
        insertTag = ' ' + insertTag;
    }
